...
class SynDataset_legacy(utils.data.Dataset):
    '''
    ...
    def __init__(self, data, device):
        super(SynDataset_legacy, self).__init__()
        self.data = data
        self.device = device
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            ...
            return torch.tensor(self.data.iloc[index].history), \
                   torch.tensor(self.data.iloc[index].result), \
