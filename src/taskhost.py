...
class TaskHost:
    ...
    def global_settings(self: Self) -> None:
        ...
        if self.opt.no_seed:
            import time
            logger.warning('For reproducibility, you need to assign a value to the random seed. If you want reproducible results, please ABORT this run ASAP and manually provide a random seed using \'--seed\'')
            logger.warning('The user doesn\'t provide a random seed. We will randomly select a number as the random seed.')
        ...
        if self.opt.dtype != 'float32':
            logger.warning(f'Explicit casting enabled! We will train our model in {self.dict_torch_dtype[self.opt.dtype]}.')
            logger.warning('Training MTPP models using lower precision may cause suboptimal results or failed training. If your model is sensitive to the precision. We recommend to stay on float32.')
