...
from src.toolbox.misc import read_yaml, move_from_tensor_to_ndarray, \
...
class EHD(BasicModule):
    ...
    def __init__(self, opt,
        ...
        # +1 to include the dummy event.
        self.seq_len_x = opt.info_dict['length_of_x'] + 1
        self.seq_len_h = opt.info_dict['length_of_h'] + 1
    ...
    def load_model(self, training):
        ...
        self.mtpp_model = model_class(device = self.opt.device, opt = self.opt,
            **model_param
        )
        ...
        if training:
            ...
            logger.info(print_args(self.opt, 'The parameter used to train the loaded MTPP checkpoint'))
    ...
    def divide_history_and_future(self, input_time, input_events, input_mask):
        ...
        '''
        gen_time_history, gen_time_next = input_time[:, :self.seq_len_h], input_time[:, self.seq_len_h:]
        gen_events_history, gen_events_next = input_events[:, :self.seq_len_h], input_events[:, self.seq_len_h:]
        gen_mask_history, gen_mask_next = input_mask[:, :self.seq_len_h], input_mask[:, self.seq_len_h:]
    ...
    def evaluate_procedure(self, input_time, input_events, input_mask, mean, std):
        '''
        ...
        for padded_distilled_events_one_batch, padded_distilled_masks_one_batch, padded_distilled_times_one_batch, padded_distilled_event_embeddings_one_batch, \
            ...
            # In the evaluation stage, we sample the mask only once, so (log_p_h_f_x_o - log_p_h_l_x_o).mean() should be the final result.
    ...
    def extract_minibatch(self, minibatch):
        ...
        * std           type: int shape: N/A
        ...
        return input_time, input_events, mask, mean, std
    ...
    def lsp_and_lrp_compared_with_gs_and_rd(self, input_data, opt):
        ...
        number of removed event stdies with the sequence length.
        '''
        import time
        ...
        for the_number_of_remained_events_per_seq, the_number_of_historical_events_per_seq \
            ...
            selected_L_sp_given_events, selected_L_rp_given_events = \
                self.get_metric_values(input_events, input_time, input_mask, initial_filter_mask, mean, std)
            ...
            while masked_events < the_number_of_historical_events_per_seq - the_number_of_remained_events_per_seq:
                ...
                L_sp_r_d, L_rp_r_d = self.get_metric_values(input_events, input_time, input_mask, filter_mask_random, mean, std, return_mean = False)
                                                                               # [number_of_sampled_sequence]
        ...
        while True:
            ...
            for the_number_of_remained_events_per_seq in random_remained_events:
                ...
                L_sp_r_d, L_rp_r_d = self.get_metric_values(input_events, input_time, input_mask, filter_mask_random, mean, std)
    ...
    def get_metric_values(self, input_events, input_time, input_mask, filter_mask, mean, std, return_mean = True):
        '''
    ...
    def train_step(model, minibatch, device):
        ...
        loss, L_c, L_p, L_g = model(task_name = 'train', input_time = time_seq, input_events = event_seq, \
                                    input_mask = mask, mean = mean, std = std)
        loss.backward()
    ...
    def evaluation_step(model, minibatch, device):
        ...
        model.eval()
        [time_seq, event_seq, score, mask], (mean, std) = minibatch
        loss, L_n, L_e, L_g, the_number_of_left_events, L_l_without_hinge, L_d_without_hinge \
            = model(
        ...
        L_l_without_hinge = L_l_without_hinge.item()
        L_d_without_hinge = L_d_without_hinge.item()
    ...
    def log_print_format(input, procedure):
        def train_log_print_format(input):
            ...
            format_dict['L_c'] = pack_one_value_to_dict(input[1])
            format_dict['L_p'] = pack_one_value_to_dict(input[2])
            format_dict['L_g'] = pack_one_value_to_dict(input[3])
            return format_dict

        def test_log_print_format(input):
            format_dict = {}
            format_dict['Loss'] = pack_one_value_to_dict(input[0])
            format_dict['L_n'] = pack_one_value_to_dict(input[1])
            format_dict['L_e'] = pack_one_value_to_dict(input[2])
            format_dict['L_g'] = pack_one_value_to_dict(input[3])
            format_dict['the_number_of_left_events'] = pack_one_value_to_dict(input[4])
            format_dict['L_l_without_hinge'] = pack_one_value_to_dict(input[5])
            format_dict['L_d_without_hinge'] = pack_one_value_to_dict(input[6])
            return format_dict
