...
class FullyNN(nn.Module):
    ...
    def __init__(self, d_history, d_intensity, dropout, num_layers, mlp_layers):
        ...
        self.rnn = nn.LSTM(input_size = 1, hidden_size = d_history, num_layers = num_layers, batch_first = True, dropout = dropout)

        self.hidden_x = NonNegLinear(1, d_intensity, bias = False)
        self.hidden_p = nn.Linear(d_history * num_layers, d_intensity, bias = True)
        ...
        self.agg = NonNegLinear(d_intensity, 1, bias = True)
