...
from src.TPP.model.tifn.transformers import TransEncoder


class TIFN(nn.Module):
    """
    TIFN (Transformer Integration-free Neural Marked Temporal Point Process)
    """
    ...
    def __init__(
        ...
        """
        This function creates a TIFN model.
        ...
        self.his_encoder = TransEncoder(
            num_marks, d_history, d_hidden, n_layers, n_head, d_qk, d_v, dropout, device=self.device
        )
    ...
    def forward(
        ...
        hidden_history = self.his_encoder(time_history, marks_history, mask_history)
    ...
    def gamma_at_t_autoregressive(self, sampled_time_history, tau, sampled_marks_history, mean, std, extend_input_time=True):
        """
        ...
        sampled_history_embedding = self.his_encoder(
            sampled_time_history, sampled_marks_history, torch.ones_like(sampled_marks_history)
        )
        # [number_of_sampled_sequences, sampled_seq_len, d_history]
        sampled_history_embedding = sampled_history_embedding[:, -1:, :]  # [number_of_sampled_sequences, 1, d_history]

        sampled_history_embedding = repeat(sampled_history_embedding, "b s dh -> b s ne dh", ne=self.num_marks)
        # [number_of_sampled_sequences, 1, num_marks, d_history]
        sampled_history_embedding = self.history_mapper(sampled_history_embedding)
        # [number_of_sampled_sequences, 1, num_marks, d_intensity]
        # Obtain timestamp embeddings.
        tau = (tau - mean) / std  # [number_of_sampled_sequences, batch_size, num_marks]
        time_next_zero = torch.ones_like(tau) * (-mean / std)  # [number_of_sampled_sequences, batch_size, num_marks]
        ...
        # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
        time_zero_embedding = time_next_zero.unsqueeze(dim=-1) * self.nonneg_activation(self.weight_for_t) + time_bias
        # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
        time_embedding = self.time_mapper(time_embedding)  # [number_of_sampled_sequences, 1, num_marks, d_intensity]
        time_zero_embedding = self.time_mapper(
            time_zero_embedding
        )  # [number_of_sampled_sequences, 1, num_marks, d_intensity]

        output = time_embedding + sampled_history_embedding  # [number_of_sampled_sequences, 1, num_marks, d_intensity]
        output_zero = (
            time_zero_embedding + sampled_history_embedding
        )  # [number_of_sampled_sequences, 1, num_marks, d_intensity]

        for layer_idx, layer in enumerate(self.mlp):
            output = layer(output)  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
            output = self.layer_activation(output)  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]

            output_zero = layer(output_zero)  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
            output_zero = self.layer_activation(
                output_zero
            )  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
            ...
            if layer_idx == 0:
                ...
                output_max = layer(output_max)  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]
                output_max = self.layer_activation(
                    output_max
                )  # [number_of_sampled_sequences, batch_size, num_marks, d_intensity]

        probability_integral_from_t_to_inf = self.nonneg_integral(-self.aggregate(output))
        # [number_of_sampled_sequences, batch_size, num_marks, 1]
        probability_integral_from_tl_to_inf = self.nonneg_integral(-self.aggregate(output_zero))
        # [number_of_sampled_sequences, batch_size, num_marks, 1]
        probability_integral_minimal = self.nonneg_integral(-self.aggregate(output_max))
        # [number_of_sampled_sequences, batch_size, num_marks, 1]
        ...
        probability_integral_from_t_to_inf = rearrange(regularized_probability_integral_from_t_to_inf, "... 1 -> ...")
        # [number_of_sampled_sequences, batch_size, num_marks]
        probability_integral_from_tl_to_inf = reduce(
            regularized_probability_integral_from_tl_to_inf, "... ne 1 -> ... ()", "sum"
        )
    ...
    def model_probe_function(
        ...
        pearson_matrix = []
        l1_matrix = []
        for _, (expand_probability_per_seq, mask_next_per_seq, original_time_expand_per_seq) in enumerate(
