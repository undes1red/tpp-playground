...
from src.TPP.model.tifn.plot import generate_debug_figure, generate_probability_figure
from src.TPP.model.tifn.sample import sample_mark_time, sample_time, sample_time_mark
...
class TIFNModel(
    ...
    def forward(self, task_name, *args, **kwargs):
        """
        Remove the probability of the dummy mark from the mask.
        ...
        ### Outputs
            * ```torch.tensor``` mask_without_dummy
              shape: [batch_size, seq_len]
              The output mask tensor with the last unmask mark in each sequence removed.
        """
        task_mapper = {
    ...
    def evaluate_procedure(self, input_time, input_marks, mask, mean, std):
        ...
        probability_integral_from_time_next_to_infinite, probability_of_all_marks = self.model(
            time_history, time_next, marks_history, mask_history=mask_history, mean=mean, std=std
        )
        # [batch_size, seq_len, num_marks] * 2
    ...
    def next_event_prediction_time_mark(
        ...
        if evaluation:
            _, probablity_all_marks = self.model(time_history, time_next, marks_history, mask_history, mean, std)
        # [batch_size, seq_len, num_marks]
        else:
            _, probablity_all_marks = self.model(time_history, pred_time, marks_history, mask_history, mean, std)
            # [batch_size, seq_len, num_marks]
    ...
    def next_event_prediction_mark_time(
        ...
        """
        mark_distribution = self.get_pm_next_event(time_history, marks_history, mask_history, 0, 0, mean, std)
        # [batch_size, seq_len, num_marks]
    ...
    def get_pm_next_event(
        ...
        mark_distribution, _ = self.model(
            time_history, time_zero, marks_history, mask_history, mean=mean, std=std
        )  # [batch_size, seq_len, num_marks]
    ...
    def figure_debug(self, input_data, opt):
        ...
        data, timestamp = self.model.model_probe_function(
            time_history, time_next, marks_history, mask_history, mask_next, opt.resolution, mean, std
        )
        ...
        sampled_data_mark_time, sampled_timestamp_mark_time = self.model.model_probe_function(
            ...
            sampled_mask_history_mark_time,
