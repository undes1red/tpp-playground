...
class CTLSTM(nn.Module):
    ...
    def forward(self, event_tensor, dtime_tensor, token_num_tensor, duration_tensor, eval_tag=False, intensity_instead = False): 
		...
		"""
        target_tensor, mask_tensor = self.get_target(event_tensor)             # [batch_size, seq_len - 1]
        ...
        self.get_cells_gates_states(event_tensor, dtime_tensor)                # [batch_size, seq_len - 1, hidden_dim], they are c, \bar{c}, \delta, and o
		...
		)                                                                      # 4 * [batch_size, (the sum of all sample_dts), hidden_dim] and 2 * [batch_size, (the sum of all sample_dts)]
        inten_p_noise = self.get_intensities_all_types(
			all_c_p_noise, all_cb_p_noise, all_d_p_noise, all_o_p_noise, 
			all_dtime_noise
		)                                                                      # [batch_size, (the sum of all sample_dts), event_num]
        log_inten = torch.log(inten_p_actual.sum(-1) + self.eps) * mask_tensor # [batch_size, seq_len - 1]
        integral = torch.sum(inten_p_noise, dim=-1) * all_mask_noise           # [batch_size, (the sum of all sample_dts)]
        actual_counts = torch.sum(all_mask_noise, dim=-1)                      # [batch_size] 
        integral = torch.sum(integral, dim=-1) / actual_counts
        integral = duration_tensor * integral                                  # [batch_size]
    ...
    def get_cells_gates_states(self, x_event, x_dtime):
        ...
        all_cell, all_cell_bar = [], []                                        # (length - 1) * [batch_size, hidden_dim], c_{i + 1} and \bar{c}_{i + 1}
        all_gate_output, all_gate_decay = [], []                               # (length - 1) * [batch_size, hidden_dim], o_{i + 1} and \delta{i + 1}
        all_hidden = []                                                        # (length - 1) * [batch_size, hidden_dim], h(t_{i + 1})
        all_hidden_after_update = []                                           # (length - 1) * [batch_size, hidden_dim], h(\delta t = 0)
        ...
        for i in range(length - 1):
			...
			# BOS, k1t1, ..., kItI
            emb_i = self.mark(x_event[:, i ])                                  # [batch_size, hidden_dim]
            dtime_i = x_dtime[:, i + 1 ] # need to carefully check here        # [batch_size]
        ...
        '''
        The dimensions of the following six tensors are listed above from line 166 to 169.
        '''
    ...
    def get_intensities_given_types(self, 
		...
		"""
        embedding = self.mark(event_tensor)                                    # [batch_size, seq_len - 1, 1, hidden_dim]
        ...
        intensities = F.softplus(
			torch.matmul(
				embedding, all_h_t.unsqueeze(-1)
			).squeeze(-1), beta=self.beta
		)                                                                      # [batch_size, seq_len - 1, 1]
    ...
    def get_mc_samples(self,
        ...
        u = torch.ones(size=[batch_size, mc_max], dtype=torch.float32, device=self.device)
                                                                               # [batch_size, mc_max] mc_max is the maximum of event_length * sample_in_each_interval
        u, _ = torch.sort(u.uniform_(0.0, 1.0))                                # [batch_size, mc_max]
        sampled_time = u * duration_tensor.unsqueeze(-1)                       # [batch_size, mc_max]
        ...
        for i in range(T_plus_1): 
            ...
            fallin = (sampled_time > last_time.unsqueeze(-1)) \
				& (sampled_time <= curr_time.unsqueeze(-1))                    # [batch_size, mc_max]
            ...
            mask_inter = fallin[:, fallin_idx].float()                         # [batch_size, fallin_idx], shrink the fallin matrix to reduce computation costs.
            mask_inter *= mask_tensor[:, i].unsqueeze(1)                       # [batch_size, fallin_idx]
            
            chosen_time = sampled_time[:, fallin_idx]                          # [batch_size, fallin_idx]
            sampled_dt = chosen_time - last_time.unsqueeze(-1)                 # [batch_size, fallin_idx]
            """
			chosen time may < past time : they are chosen cuz that col is chosen 
			and they will eventually be masked out in the end 
			"""
            sampled_dt[sampled_dt < 0.0] = 0.0                                 # [batch_size, fallin_idx]
			# batch_size x S (S may be 0) My idea: It could be a bad sign if S = 0.
            _, S = sampled_dt.size()
            ...
            o_inter = all_gate_output[:, i, :].unsqueeze(1).expand(batch_size, S, hidden_dim)
                                                                               # [batch_size, fallin_idx]
            
            last_time = curr_time

            all_c_inter.append(c_inter)                                        # [batch_size, S, hidden_dim]
            all_cb_inter.append(cb_inter)                                      # [batch_size, S, hidden_dim]
            all_d_inter.append(d_inter)                                        # [batch_size, S, hidden_dim]
            all_o_inter.append(o_inter)                                        # [batch_size, S, hidden_dim]

            all_dtime_inter.append(sampled_dt)                                 # [batch_size, S]
            all_mask_inter.append(mask_inter)                                  # [batch_size, S]
    ...
    def get_intensities_all_types(self, 
		...
		"""
        all_embs = self.get_embeddings_all_types()                             # [event_num, hidden_dim]
		# C x D 
        _, all_h_t = self.rnn_cell.decay(
			all_cell, all_cell_bar, all_gate_decay, all_gate_output, dtime_tensor)
		# batch_size x T+1 x D                                                 # [batch_size, (the sum of all sample_dts), hidden_dim]
        all_intensities = F.softplus(
			torch.matmul(all_h_t, all_embs.t()), beta=self.beta)
		# batch_size x T+1 x C                                                 # [batch_size, (the sum of all sample_dts), event_num]
