...
from src.toolbox.misc import move_from_tensor_to_ndarray, stable_palette, figure_instruction_generator, save_fig, get_logger
...
def generate_intensity_figure(data, timestamp, opt):
    ...
    for idx, (expand_intensity_per_seq, events_next_per_seq, time_next_per_seq, mask_next_per_seq, timestamp_per_seq, true_intensity_per_seq) \
        ...
        fig = draw_intensity_integral_and_probability(df_intensity_plot, df_event, annotation, 'Intensity', color_palette, num_events)
        save_fig(fig, os.path.join(opt.plot_store_dir_for_this_batch, f'intensity_{idx}.pdf'))
...
def generate_integral_figure(data, timestamp, opt):
    ...
    for idx, (expand_integral_per_seq, events_next_per_seq, time_next_per_seq, mask_next_per_seq, timestamp_per_seq) \
        ...
        df_integral_plot = pd.DataFrame.from_dict(
                {'Time': timestamp_per_seq.flatten(),
                 'Predicted': expand_integral_per_seq[:seq_len, :].flatten()}
        )

        fig = draw_intensity_integral_and_probability(df_integral_plot, df_event, None, 'Integral', color_palette, num_events)
        save_fig(fig, os.path.join(opt.plot_store_dir_for_this_batch, f'integral_{idx}.pdf'))
...
def generate_probability_figure(data, timestamp, opt):
    ...
    for idx, (expand_probability_per_seq, events_next_per_seq, time_next_per_seq, mask_next_per_seq, timestamp_per_seq, true_probability_per_seq) \
        ...
        save_fig(fig, os.path.join(opt.plot_store_dir_for_this_batch, f'probability_{idx}.pdf'))
...
def generate_debug_figure(data, timestamp, opt):
    ...
    for idx, (events_next_per_seq, time_next_per_seq, mask_next_per_seq, expand_intensity_per_seq, \
        ...
        save_fig(fig1, os.path.join(opt.plot_store_dir_for_this_batch, f'mark_wise_intensity_{idx}.pdf'))
        ...
        save_fig(fig2, os.path.join(opt.plot_store_dir_for_this_batch, f'mark_wise_integral_{idx}.pdf'))
    ...
    for value in ['spearman', 'pearson', 'L1']:
        ...
        for idx, matrix in enumerate(matrices):
            ...
            save_fig(fig, os.path.join(opt.plot_store_dir_for_this_batch, f'{value}_{idx}.pdf'))
    ...
    for idx, top_k_per_seq in enumerate(top_k):
        ...
        save_fig(fig6, os.path.join(opt.plot_store_dir_for_this_batch, f'topk_{idx}.pdf'))
    ...
    for idx, (mae_per_seq, mae_per_event_with_predict_index_per_seq, mae_per_event_with_event_next_per_seq, mask_next_per_seq) in enumerate(packed_data):
        ...
        save_fig(fig7, os.path.join(opt.plot_store_dir_for_this_batch, f'MAE_{idx}.pdf'))
    ...
    for idx, (probability_sum_per_seq, mask_next_per_seq) in enumerate(packed_data):
        ...
        ax = fig8.gca()
        ax.set_ylim(bottom =  -0.05, top = 1.05)
        save_fig(fig8, os.path.join(opt.plot_store_dir_for_this_batch, f'sum_of_p_{idx}.pdf'))
    ...
    for idx, (tau_pred_all_event_per_seq, mask_next) in enumerate(zip(tau_pred_all_event, mask_next)):
        ...
        save_fig(fig9, os.path.join(opt.plot_store_dir_for_this_batch, f'log_pred_time_{idx}.pdf'))
