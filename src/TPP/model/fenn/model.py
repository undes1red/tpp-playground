...
from src.TPP.model.fenn.sample import sample_time
from src.TPP.model.fenn.submodel import FENN
from src.TPP.model.utils import (
    decide_resolution_inf_and_resolution_between_marks,
...
class FENNModel(BasicModel):
    """
    The FENN(Fully Event Neural Network), an intuitive variation of FullyNN which learns the MTPP.

    However, the mark prediction performance of FENN is marginally better than FullyNN, the time prediction performance
    is not guaranteed to be better. Sometimes, FENN predicts time worse than FullyNN.
    """
    ...
    def train_procedure(self, input_time, input_marks, mask, mean, std):
        """
        FENN's forwardpropagation function for training.
        ...
        loss = log_likeli_loss_time_next_without_dummy + loss_survival
    ...
    def evaluate_procedure(self, input_time, input_marks, mask, mean, std):
        """
        FENN's forwardpropagation function for evaluation.
        ...
        # L = -log \\frac{\\partial \\Lambda^*(m, t)}{\\partial t} + \\Lambda^*(m, t)
        log_likeli_loss_time_next_without_dummy, marker_loss_time_next_without_dummy = self.loss_function(
            integral_all_marks=integral_for_each_event_from_tl_to_time_next,
            intensity_all_marks=intensity_for_each_event_at_time_next,
            marks_next=marks_next_without_dummy,
            mask_next=mask_next_without_dummy,
        )
        # Survival probability: \\int_{t_N}^{T}{\\sum_{k}\\lambda_k^(\\tau)d\\tau}
        dummy_event_index = mask_next.sum(dim=-1) - 1  # [batch_size]
        integral_survival = integral_for_each_event_from_tl_to_time_next.sum(dim=-1).gather(
            index=dummy_event_index.unsqueeze(dim=-1), dim=-1
        )
        # [batch_size, 1]
    ...
    def cppod_commission_evaluation(self, input_data, opt):
        ...
        integral_for_each_event_from_tl_to_time_next = self.model(
            marks_history, time_history, time_next, mean=mean, std=std
        )
        # [batch_size, seq_len, num_marks]
        """
        ...
        check_tensor(intensity_for_each_event_from_tl_to_time_next)  # [batch_size, seq_len, num_marks]
    ...
    def ehd_perplexity(
        ...
        log_probability = torch.log(intensity + self.epsilon) - padded_filtered_intensity_integral_from_t_o_to_t.sum(
            dim=-1
        )
        # [batch_size, filtered_seq_len - 1]
