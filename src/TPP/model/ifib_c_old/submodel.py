...
from src.TPP.model.ifib_c_old.nonneg import NonNegLinear
...
class IFIBC(nn.Module):
    ...
    def __init__(self, d_history, d_intensity, num_events, dropout, history_module, history_module_layers,
        ...
        if pretrain:
            ...
            # alpha
            self.output_factor = nn.Parameter(torch.tensor(alpha,  device = self.device), requires_grad = False)
            # beta
            self.residual_factor = nn.Parameter(torch.tensor(beta, device = self.device), requires_grad = False)
