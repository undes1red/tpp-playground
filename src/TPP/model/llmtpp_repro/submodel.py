...
from src.TPP.model.llmtpp_repro.transformers_module import lm_module_location
from src.TPP.model.llmtpp_repro.embedding import DataEmbedding
from src.TPP.model.llmtpp_repro.reprogramming_functions import ReprogramInput, ReprogramOutput


class LLMTPP(nn.Module):
    def __init__(self, num_events, llm_class_name, full_llm_name, d_model, d_embedding, lm_layers, device, dropout):
        ...
        self.squeezed_token_embedding = nn.Linear(self.vocab_size, 1000)
        ...
        self.time_seq_to_token_emb = ReprogramInput(n_head = 3, d_lm_embedding = self.d_lm_embedding, \
                                                    d_hidden = 3 * self.d_lm_embedding, device = self.device)
    ...
    def model_forward(self, events_history, time_history, mask_history, mean, std):
        ...
        outputs = self.retrieved_lm(inputs_embeds = converted_emb).hidden_states[-1]
                                                                               # [batch_size, seq_len, d_lm_model]
