...
class FullyNN(nn.Module):
    ...
    def forward(self, events_history, time_history, time_next, mean, var, mask):
        '''
        Args:
            events_history: [batch_size, seq_len, history_length]
            time_history:   [batch_size, seq_len, history_length]
            time_next:      [batch_size, seq_len]
            mask:           [batch_size, seq_len, history_length]
        '''
        # Input data normalization
        batch_size, seq_len = time_next.shape
        time_history = (time_history - mean) / var                             # [batch_size, seq_len, history_length]
        time_next = (time_next - mean) / var                                   # [batch_size, seq_len]
        time_next = time_next.unsqueeze(dim = -1)                              # [batch_size, seq_len, 1]
        
        # Reshape hidden output for full connection layers.
        if self.history_module == 'lstm':
            if self.event_toggle:
                events_embeddings = self.events(events_history)                # [batch_size, seq_len, history_length, d_history]
                history = torch.cat(
                    (events_embeddings, time_history.unsqueeze(dim = -1)), dim = -1
                )
            else:
                history = time_history                                         # [batch_size, seq_len, history_length, d_history + 1] if we need events else [batch_size, seq_len, history_length, 1]
            
            history = rearrange(history, 'b s h d -> (b s) h d')               # [batch_size * seq_len, history_length, d_history + 1]
            _, (h_n, _) = self.his_encoder(history)                            # [num_layer, batch_size * seq_len, d_history]
            output = rearrange(h_n, 'n bs d -> bs d n')
                                                                               # [batch_size * seq_len, d_history, num_layer]
            # Perform average pooling here.
            output = self.pooling(output)                                      # [batch_size * seq_len, d_history, 1]
            output = rearrange(output, '(b s) d n -> b s d n', b = batch_size) # [batch_size, seq_len, d_history, 1]
            output = output.squeeze(dim = -1)                                  # [batch_size, seq_len, d_history]
