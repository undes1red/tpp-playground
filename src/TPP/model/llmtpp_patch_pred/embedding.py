...
class DataEmbedding(nn.Module):
    def __init__(self, num_events, d_embedding, d_model, num_events_in_one_patch, dropout = 0.1, device = None):
    ...
    def forward(self, x, time, mask):
        mark_embedding = self.mark_embedding(x) * mask.unsqueeze(dim = -1)       # [batch_size, num_of_patch, patch_len, d_emb]
        time_embedding = self.time_embedding(time) * mask.unsqueeze(dim = -1)    # [batch_size, num_of_patch, patch_len, d_emb]
        position_embedding = self.position_embedding(x)                          # [batch_size, num_of_patch, d_model]

        x = self.to_token_embedding(mark_embedding + time_embedding) + position_embedding
