...
from src.TPP.model.utils import median_prediction, step_split
...
def sampling_by_its_for_tm(self, marks_history, time_history, number_of_total_samples, step, mean, std):
    ...
    def bisect_target(taus, probability_threshold):
        integral = self.model(time_history, marks_history, taus, mean, std)
        # [sample_rate, batch_size, seq_len, num_marks]
    ...
    for sub_sample_rate in sample_rate_list:
        ...
        tau_pred.append(
            median_prediction(self.max_step, self.bisect_early_stop_threshold, bisect_target, probability_threshold)
        )
        # [sample_rate, batch_size, seq_len]
