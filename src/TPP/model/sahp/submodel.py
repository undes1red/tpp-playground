...
class SAHP(nn.Module):
    def __init__(
        ...
        self.intensity_layer = nn.Sequential(
            nn.Linear(d_input, self.num_events, bias=True, device=self.device), nn.Softplus(beta=1.0).to(torch.bfloat16)
        )
    ...
    def state_decay(self, mu, eta, gamma, duration_t, num_dimension_prior_batch):
        ...
        ### Outputs
            * ```torch.tensor``` cell_t
              shape: ```[..., batch_size, seq_len, (integration_sample_rate, num_events), d_input]```
        ...
        duration_t = duration_t.unsqueeze(
            dim=-1
        )  # [..., batch_size, seq_len, (integration_sample_rate, num_events), 1]
    ...
    def forward(
        ...
        integral_all_events = approximate_integration(
            expanded_intensity_all_events, expanded_time, dim=-2, only_integral=True
        )
