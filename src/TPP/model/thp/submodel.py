...
class THP(nn.Module):
    def __init__(
        ...
        self.alpha = nn.Parameter(
            torch.ones((self.num_events), device=self.device, requires_grad=True)
        )
        nn.init.normal_(self.alpha)

        # parameter for the softplus function
        self.beta = nn.Parameter(
            torch.zeros((self.num_events), device=self.device, requires_grad=True)
        )
    ...
    def forward(self, time_history, time_next, events_history, mask_history):
        ...
        intensity_all_events_pre_softplus = repeat(
            intensity_all_events_pre_softplus, "... ne -> ... r ne", r=self.integration_sample_rate
        )
        # [..., batch_size, seq_len, integration_sample_rate, num_events]
    ...
    def integral_intensity_time_next_2d(
        self, events_history, time_history, time_next, mask_history, integration_sample_rate, time_next_start=None
    ):
        """
        ...
        expanded_intensity_all_events = softplus_ext(
    ...
    def integral_intensity_time_next_3d(
        self, events_history, time_history, time_next, mask_history, integration_sample_rate
    ):
        """
        Probe the value of the intensity function and its integral at sampled timestamps.
        ...
        expanded_intensity_across_all_events = softplus_ext(
        ...
        # [..., batch_size, seq_len, num_events, integration_sample_rate, num_events]
        expanded_integral_across_all_events = approximate_integration(
            expanded_intensity_across_all_events, original_expanded_time, dim=-2
        )
        # [..., batch_size, seq_len, num_events, integration_sample_rate, num_events]
    ...
    def model_probe_function(
        self, events_history, time_history, time_next, mask_history, mask_next, integration_sample_rate
    ):
        """
        Probe the value of the intensity function and its integral at sampled timestamps.
        In this function, all marks can have their sampled timestmaps, so the dimension of time_next is ```[..., batch_size, seq_len, num_events]```.
        ...
        data["expand_intensity_for_each_event"] = (
            expanded_intensity_all_events  # [batch_size, seq_len, integration_sample_rate, num_events]
        )
        data["expand_integral_for_each_event"] = (
            expanded_integral_all_events  # [batch_size, seq_len, integration_sample_rate, num_events]
        )
        ...
        for idx, (expand_intensity_per_seq, expand_integral_per_seq, mask_per_seq, expanded_time_per_seq) in enumerate(
            ...
            # L^1 metric
            L1_matrix_per_seq = L1_distance_across_events(
