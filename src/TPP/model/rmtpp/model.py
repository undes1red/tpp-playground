...
class RMTPP(
    ...
    """
    We combine multiple RMTPP modules to estimate the joint distribution p^*(m, t).
    ...
    def __init__(
        ...
        """
        This function creates a RMTPP model.
        ...
        self.model = RMTPPModule(
            ...
            limited_history_norm=limited_history_norm,
            time_scalar_min=time_scalar_min,
            device=device,
        )
    ...
    def train_procedure(self, time, marks, mask, mean, std):
        ...
        ### Outputs
            ...
            * ```torch.tensor``` marks_loss_without_dummy
              shape: ```[1]```
        ...
        integral, intensity = self.model(time_history, time_next, marks_history, mean, std)
        # [batch_size, seq_len, num_marks] * 2

        # loss_without_dummy = time_loss_without_dummy + marks_loss_without_dummy
        # time_loss_without_dummy = \\sum_{t_i}{\\lambda^*(t_i)} + \\int_{t_0}^{t_n}{\\lambda^*(\\tau)d\\tau}
        # mark_loss_without_dummy = \\sum{x_i}{CrossEntropyLoss(\\hat{x_i}, x_i)} for all real-world marks.
        time_loss_without_dummy, marks_loss_without_dummy = self.loss_function(
            intensity, integral, marks_next_without_dummy, mask_next_without_dummy
        )
        ...
        if self.survival_loss_during_training:
            ...
            probability_survival = integral.sum(dim=-1).gather(index=dummy_mark_index.unsqueeze(dim=-1), dim=-1).sum()

        training_loss = time_loss_without_dummy + probability_survival

        return training_loss, time_loss_without_dummy, marks_loss_without_dummy, the_number_of_marks

    @torch.inference_mode()
    def evaluate_procedure(self, time, marks, mask, mean, std):
        """
        RMTPP's forwardpropagation function for evaluation.
        ...
        ### Outputs
            ...
            * ```torch.tensor``` marks_loss_time_next_without_dummy
              shape: ```[1]```
              The sum of the mark loss: L = -log \\frac{\\lambda^*(m, t)}{\\sum_{n \\in M}{\\lambda^*(n, t)}} where m is the mark of the real mark.
            * ```float``` mae
        ...
        integral_time_next, intensity_time_next = self.model(time_history, time_next, marks_history, mean, std)
        # [batch_size, seq_len, num_marks]

        # NLL and mark loss at time_next.
        log_likeli_loss_time_next_without_dummy, marker_loss_time_next_without_dummy = self.loss_function(
            intensity_time_next, integral_time_next, marks_next_without_dummy, mask_next_without_dummy
        )
        # survival loss: \\int_{t_n}^{T}{\\lambda^*(\\tau)d\\tau}
        dummy_mark_index = mask_next.sum(dim=-1) - 1  # [batch_size]
        loss_survival = integral_time_next.sum(dim=-1).gather(index=dummy_mark_index.unsqueeze(dim=-1), dim=-1).mean()
    ...
    def loss_function(self, intensity, integral, marks_next, mask_next):
        ...
        intensity_mask = torch.nn.functional.one_hot(marks_next.long(), num_classes=self.num_marks)
        # [batch_size, seq_len, num_marks]
        ...
        log_probability_for_each_mark = torch.log(intensity + self.epsilon)  # [batch_size, seq_len, num_marks]
        marks_probability = torch.nn.functional.softmax(log_probability_for_each_mark, dim=-1)
        # [batch_size, seq_len, num_marks]
        marks_loss = torch.nn.functional.cross_entropy(
            rearrange(marks_probability, "b s ne -> b ne s"), marks_next.long(), reduction="none"
        )
        # [batch_size, seq_len]
        marks_loss = marks_loss * mask_next  # [batch_size, seq_len]
        marks_loss = marks_loss.sum()

        return time_loss, marks_loss
    ...
    def next_event_prediction_time_mark(
        ...
        if evaluation:
            _, intensity_all_marks = self.model(time_history, time_next, marks_history, mean, std)
        # [batch_size, seq_len, num_marks]
        else:
            _, intensity_all_marks = self.model(time_history, pred_time, marks_history, mean, std)
            # [batch_size, seq_len, num_marks]
