...
class RMTPPModule(nn.Module):
    def __init__(
        ...
        ### Args
            * ```int``` input_size
              The dimension of the mark representation that is fed into the history encoder.
            * ```int``` hidden_size
        ...
        # Mark related
        self.mark_embedding = nn.Embedding(
            num_embeddings=num_marks + 1, embedding_dim=input_size, padding_idx=num_marks, device=self.device
        )

        self.non_neg_activation = nn.Softplus()

        self.intensity = nn.Linear(output_size, self.num_marks, device=self.device)
        self.time_scalar = nn.Linear(output_size, self.num_marks, device=self.device)
        self.base_intensity = nn.Linear(output_size, self.num_marks, device=self.device)
    ...
    def clamp_time_scalar(self, time_scalar):
        ...
        """
        time_scalar_sign = (time_scalar >= 0).int() - (time_scalar < 0).int()  # [batch_size, seq_len, num_marks]
        shifted_time_scalar_abs_value = torch.abs(time_scalar).clamp(min=self.time_scalar_min)
        # [batch_size, seq_len, num_marks]
        return shifted_time_scalar_abs_value * time_scalar_sign  # [batch_size, seq_len, num_marks]

    def forward(self, time_history, time_next, marks_history, mean, std, custom_marks_history=False):
        """
        ...
        marks_vec = (
            marks_history if custom_marks_history else self.mark_embedding(marks_history)
        )  # [batch_size, seq_len, input_size]
        input_vec = time_vec + marks_vec
        ...
        history_part = self.intensity(hidden_history)  # [batch_size, seq_len, num_marks]

        if self.limited_history_norm:
            history_part = torch.tanh(history_part)  # [batch_size, seq_len, num_marks]

        history_part = torch.exp(history_part)  # [batch_size, seq_len, num_marks]

        constant = history_part * torch.exp(self.base_intensity(hidden_history))
        # [batch_size, seq_len, num_marks]
        time_scalar = self.time_scalar(hidden_history)  # [batch_size, seq_len, num_marks]

        # time_scalar can not be zero.
        time_scalar = self.clamp_time_scalar(time_scalar)  # [batch_size, seq_len, num_marks]
        ...
        time_scalar = rearrange(time_scalar, ein_ops)  # [..., batch_size, seq_len, num_marks]
        constant = rearrange(constant, ein_ops)  # [..., batch_size, seq_len, num_marks]

        # Get the intensity function and corresponding integral.
        intensity = torch.exp(time_scalar * time_next) * constant  # [..., batch_size, seq_len, num_marks]
        integral = (intensity - constant) / time_scalar * std  # [..., batch_size, seq_len, num_marks]

        return integral, intensity

    def get_mark_embedding(self, input_mark):
        return self.mark_embedding(input_mark)  # [batch_size, seq_len, input_size]

    def integral_intensity_time_next_2d(self, time_history, time_next, marks_history, integration_sample_rate, mean, std):
        """
        ...
        time_vec = self.time_embedding(time_history)  # [batch_size, seq_len, input_size]
        marks_vec = self.mark_embedding(marks_history)  # [batch_size, seq_len, input_size]
        input_vec = time_vec + marks_vec  # [batch_size, seq_len, input_size]
        ...
        history_part = self.intensity(history_output)  # [batch_size, seq_len, num_marks]
        ...
        time_scalar = self.time_scalar(history_output).unsqueeze(dim=-1)  # [batch_size, seq_len, num_marks, 1]

        # time_scalar can not be zero.
        time_scalar = self.clamp_time_scalar(time_scalar)  # [batch_size, seq_len, num_marks, 1]
        ...
        intensity_marks = (
            torch.exp(time_scalar * expanded_time) * constant
        )  # [batch_size, seq_len, num_marks, resolution]
        integral_marks = (
            (intensity_marks - constant) / time_scalar * std
        )  # [batch_size, seq_len, num_marks, resolution]

        intensity = rearrange(intensity_marks, "b s ne r -> b s r ne")  # [batch_size, seq_len, resolution, num_marks]
        integral = rearrange(integral_marks, "b s ne r -> b s r ne")  # [batch_size, seq_len, resolution, num_marks]
    ...
    def integral_intensity_time_next_3d(self, time_history, time_next, marks_history, resolution, mean, std):
        ...
        ### Args
            ...
            * ```float``` std
              Used for input time scaling.
        ### Outputs
            * ```torch.tensor``` integral_marks
              shape: ```[..., batch_size, seq_len, resolution, num_marks]```
              The value of \\Lambda^*(m, t) at sampled times.
            * ```torch.tensor``` intensity_marks
              shape: ```[..., batch_size, seq_len, resolution, num_marks]```
              The value of \\lambda^*(m, t) at sampled times.
            * ```torch.tensor``` original_time_expand
              shape: ```[..., batch_size, seq_len, resolution]```
        ...
        # [batch_size, seq_len, num_marks]
        time_scalar = self.time_scalar(history_output)  # [batch_size, seq_len, num_marks]

        # time_scalar can not be zero.
        time_scalar = self.clamp_time_scalar(time_scalar)  # [batch_size, seq_len, num_marks]

        einop = f"... ne -> {'() ' * (len(time_next.shape) - 3)}... () () ne"
        constant = rearrange(constant, einop)  # [..., batch_size, seq_len, 1, 1, num_marks]
        time_scalar = rearrange(time_scalar, einop)  # [..., batch_size, seq_len, 1, 1, num_marks]

        time_multiplier = torch.linspace(0, 1, resolution, device=self.device)
        original_time_expand = (
            time_next.unsqueeze(dim=-1) * time_multiplier
        )  # [..., batch_size, seq_len, num_marks, resolution]
        original_time_expand_normed = original_time_expand / std  # [..., batch_size, seq_len, num_marks, resolution]
        expanded_time = original_time_expand_normed.unsqueeze(
            dim=-1
        )  # [..., batch_size, seq_len, num_marks, resolution, 1]
        ...
        return integral_marks, intensity_marks, original_time_expand
    ...
    def model_probe_function(self, time_history, time_next, marks_history, mask_next, resolution, mean, std):
        ...
        intensity_marks = torch.exp(time_scalar * expanded_time) * constant  # [batch_size, seq_len, 1, resolution]
        integral_marks = (intensity_marks - constant) / time_scalar * std  # [batch_size, seq_len, 1, resolution]
        ...
        data = {}
        data["expand_intensity_for_each_mark"] = intensity  # [batch_size, seq_len, resolution, num_marks]
        data["expand_integral_for_each_mark"] = integral  # [batch_size, seq_len, resolution, num_marks]

        expand_intensity = rearrange(
            intensity, "b s r ne -> b (s r) ne"
        )  # [batch_size, seq_len * integration_sample_rate, num_mark]
        expand_integral = rearrange(
            integral, "b s r ne -> b (s r) ne"
        )  # [batch_size, seq_len * integration_sample_rate, num_mark]
        ...
        for idx, (
            ...
            l1_matrix_per_seq = L1_distance_across_marks(
                ...
                original_time_expand_per_seq[:seq_len],
                has_flatten=True,
            )
