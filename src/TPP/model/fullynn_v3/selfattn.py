import torch
import torch.nn as nn
import torch.nn.functional as F

class SelfAttn(nn.Module):
    '''
    SelfAttn module, the heart of transformers' layer
    '''
    def __init__(self, temperature, attn_dropout, device, wq_nonneg, wk_nonneg, wv_nonneg):
        super(SelfAttn, self).__init__()
        self.device = device
        self.temperature = temperature
        self.wq_nonneg = wq_nonneg
        self.wk_nonneg = wk_nonneg
        self.wv_nonneg = wv_nonneg

        self.dropout = nn.Dropout(attn_dropout)

        if self.wq_nonneg or self.wk_nonneg:
            self.shift = nn.Parameter(torch.tensor(1., requires_grad = True, device = self.device))

    def forward(self, q, k, v, mask = None):
        '''
        Args:

        1. q: input tensor. shape: [..., seq_len, n_head, d_qk]
        2. k: input tensor. shape: [..., 1, n_head, d_qk]
        3. v: input tensor. shape: [..., seq_len, n_head, d_v]
        4. mask: mask_out several values in the attention matrices. shape: [seq_len, 1]

        Output:
        1. output: the result of self attention. shape: [..., n_head, 1, d_v]
        '''

        q /= self.temperature                                                  # [..., seq_len, n_head, d_qk]

        attn = torch.einsum('...jkl, ...mkl -> ...kjm', q, k)                  # [..., n_head, seq_len, 1]

        if mask is not None:
            if self.wq_nonneg or self.wk_nonneg:
                # be compatible with the new activation function.
                attn = attn.masked_fill(mask.unsqueeze(-3), 0)                 # [..., n_head, seq_len, 1]
            else:
                attn = attn.masked_fill(mask.unsqueeze(-3), -1e9)              # [..., n_head, seq_len, 1]
        
        if self.wq_nonneg or self.wk_nonneg:
            # Tanh() or other activations?
            attn = self.dropout(torch.log(1 + self.shift * attn))              # [..., n_head, seq_len, 1]
        else:
            attn = self.dropout(F.softmax(attn, dim = -1))                     # [..., n_head, seq_len, 1]
        
        if self.wv_nonneg and not self.wq_nonneg and not self.wk_nonneg:
            attn = F.softplus(attn)                                            # [..., n_head, seq_len, 1]

        output = torch.einsum('...jkl, ...kjn -> ...jln', attn, v)             # [..., n_head, 1, d_v]

        return output, attn