...
from src.TPP.model.llmtpp_imputation.transformers_module import lm_module_location
from src.TPP.model.llmtpp_imputation.embedding import DataEmbedding
...
class LLMTPP(nn.Module):
    def __init__(self, llm_class_name, full_llm_name, patch_size, d_model, \
        ...
        self.enc_embedding = DataEmbedding(self.num_events + 2, d_embedding, d_model, self.patch_size, dropout = dropout, device = self.device)
    ...
    def patchify(self, input_events, input_time, input_mask, imputation_mask):
        # Split the input sequence into several patches.
        seq_len = input_events.shape[-1]
        ...
        input_events = torch.nn.functional.pad(input_events, p1d, 'constant', self.num_events)
                                                                               # [batch_size, num_of_patches * self.patch_size]
        input_time = torch.nn.functional.pad(input_time, p1d, 'constant', -1.0)# [batch_size, num_of_patches * self.patch_size]
        input_mask = torch.nn.functional.pad(input_mask, p1d, 'constant', 0)   # [batch_size, num_of_patches * self.patch_size]
        imputation_mask = torch.nn.functional.pad(imputation_mask, p1d, 'constant', 0)
                                                                               # [batch_size, num_of_patches * self.patch_size]
        
        input_events = rearrange(input_events, 'b (np ps) -> b np ps', np = num_of_patches)
                                                                               # [batch_size, num_of_patches, self.patch_size]
        input_time = rearrange(input_time, 'b (np ps) -> b np ps', np = num_of_patches)
                                                                               # [batch_size, num_of_patches, self.patch_size]
        input_mask = rearrange(input_mask, 'b (np ps) -> b np ps', np = num_of_patches)
                                                                               # [batch_size, num_of_patches, self.patch_size]
        imputation_mask = rearrange(imputation_mask, 'b (np ps) -> b np ps', np = num_of_patches)
                                                                               # [batch_size, num_of_patches, self.patch_size]
        
        return input_events, input_time, input_mask, imputation_mask
    ...
    def model_forward(self, input_events, input_time, input_mask, imputation_mask, mean, var):
        input_time = (input_time - mean) / var                                 # [batch_size, seq_len]
        input_events, input_time, input_mask, imputation_mask = self.patchify(input_events, input_time, input_mask, imputation_mask)
                                                                               # [batch_size, num_of_patches, patch_size]
        input_embs = self.enc_embedding(input_events, input_time, input_mask)  # [batch_size, num_of_patches, d_model]
        ...
        pred_time = (pred_time - mean / var) * var + mean                      # [batch_size, seq_len, patch_size]

        return pred_time, mark_dist, input_mask
