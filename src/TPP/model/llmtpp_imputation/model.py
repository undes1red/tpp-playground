...
from src.TPP.model.llmtpp_imputation.submodel import LLMTPP
from src.TPP.model.utils import *
from src.TPP.model.llmtpp_imputation.plot import *
...
class LLMTPPModel(BasicModel):
    def __init__(self, info_dict, llm_class_name, full_llm_name, patch_size, d_model, \
                ...
                lambda_t = 1.0, lambda_e = 1.0, mask_rate = 0.5):
    ...
    def remove_dummy_event_from_mask(self, mask, reference_tensor = None):
        ...
        for idx, (mask_per_seq, reference_mask_per_seq) in enumerate(zip(mask, reference_tensor)):
            ...
            mask_without_dummy_per_seq[0] = 0
    ...
    def train_procedure(self, input_time, input_events, input_score, input_mask, mean, var):
        _, seq_len = input_events.shape
        damaged_input_time, damaged_input_events, damaged_input_mask \
            = self.random_mask(True, True, input_time = input_time, input_events = input_events, input_mask = input_mask)
                                                                               # [batch_size, seq_len] * 4
        pred_time, pred_event_prob, patched_input_mask \
            = self.model('train', input_events = damaged_input_events, \
                         input_time = damaged_input_time, input_mask = input_mask, imputation_mask = damaged_input_mask,
        ...
        pred_event_prob = rearrange(pred_event_prob, 'b np lp nm -> b (np lp) nm')
                                                                               # [batch_size, patch_len * patch_size, num_events]
        patched_input_mask = rearrange(patched_input_mask, 'b np lp -> b (np lp)')
                                                                               # [batch_size, patch_len * patch_size]

        # Calculate the value of losses and other metrics.
        imputation_mask = (damaged_input_events == self.num_events + 1).int()  # [batch_size, seq_len]
        imputation_mask = patched_input_mask[..., :seq_len] & imputation_mask  # [batch_size, seq_len]
        mask_next_without_dummy = self.remove_dummy_event_from_mask(input_mask)# [batch_size, seq_len]
        input_events_without_dummy = input_events * mask_next_without_dummy    # [batch_size, seq_len]
        events_loss_without_dummy = torch.nn.functional.cross_entropy(rearrange(pred_event_prob[..., :seq_len, :], 'b s ne -> b ne s'), \
                                                                                input_events_without_dummy.long(), reduction = 'none')
                                                                               # [batch_size, seq_len]
        events_loss_without_dummy = events_loss_without_dummy * imputation_mask# [batch_size, seq_len]
        events_loss_without_dummy = events_loss_without_dummy.sum()

        mae = torch.abs(input_time - pred_time[..., :seq_len])
        time_loss_without_dummy = (mae * imputation_mask).sum()
        the_number_of_missing_events = imputation_mask.sum().item()

        loss = self.lambda_t * time_loss_without_dummy + self.lambda_e * events_loss_without_dummy

        return loss, time_loss_without_dummy, events_loss_without_dummy, the_number_of_missing_events
    ...
    def evaluate_procedure(self, input_time, input_events, input_score, input_mask, mean, var):
        ...
        pred_time, pred_event_prob, patched_input_mask \
            = self.model('evaluate', input_events = damaged_input_events, \
                         input_time = damaged_input_time, input_mask = input_mask, imputation_mask = damaged_input_mask,
                         mean = mean, var = var)                               # [batch_size, patch_len, patch_size] + [batch_size, patch_len, patch_size, num_events]
        pred_time = rearrange(pred_time, 'b np lp -> b (np lp)')               # [batch_size, patch_len * patch_size]
        pred_event_prob = rearrange(pred_event_prob, 'b np lp nm -> b (np lp) nm')
        ...
        time_loss_without_dummy = (mae * imputation_mask).sum()
        the_number_of_missing_events = imputation_mask.sum().item()
        mean_mae = time_loss_without_dummy.item() / the_number_of_missing_events

        loss = self.lambda_t * time_loss_without_dummy + self.lambda_e * events_loss_without_dummy
        all_predicted_events = torch.argmax(pred_event_prob[..., :seq_len, :], dim = -1)
                                                                               # [batch_size, seq_len]
        predicted_events_for_missing_events = all_predicted_events[imputation_mask == 1]
        true_events_for_missing_events = input_events[imputation_mask == 1]
        predicted_events_for_missing_events, true_events_for_missing_events = \
            move_from_tensor_to_ndarray(predicted_events_for_missing_events, true_events_for_missing_events)
        f1_pred = f1_score(y_pred = predicted_events_for_missing_events, y_true = true_events_for_missing_events, average = 'macro')

        return loss, time_loss_without_dummy, events_loss_without_dummy, f1_pred, mean_mae, the_number_of_missing_events
    ...
    def log_print_format(input, procedure):
        ...
        def test_log_print_format(input):
            ...
            format_dict['mae'] = pack_one_value_to_dict(input[4])
            return format_dict
