...
class SynAregDataset(utils.data.Dataset):
    ...
    def __init__(self, data, device, num_events, history_length = 48, plot = False, shift = False, input_norm_data = False):
        ...
        '''
        self.data = transform_autoregression(data, self.history_length)
    ...
    def __call__(self, data):
        ...
        For SynAregDataset, the padding events generated by transform_autoregression() should be masked out.
        '''
        padded_data = []
        for item in data:
            mask = np.array(item[1] <= self.num_events).astype(int)
            padded_item = [*item, mask]
