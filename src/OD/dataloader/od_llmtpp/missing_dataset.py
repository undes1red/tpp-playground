...
from src.OD.dataloader.od_llmtpp.utils import convert_missing_mask_to_gap_mask, remove_the_last_event_from_mask


class missing_dataset(utils.data.Dataset):
    '''
    OD dataset specifically for LLMTPP.
    '''
    ...
    def data_collator(self, data):
        ...
        '''
        max_length_of_this_batch = max([item[2][0].shape[-1] for item in data])
        # mask = []
        
        padded_obs_time = []
        padded_obs_time_backward = []
        padded_obs_event = []
        padded_obs_mask = []
        
        padded_interval_has_missing = []
        padded_interval_has_missing_mask = []
        ...
        for item in data:
            ...
            # In OD, LLMTPP only needs the observed sequence and backward time.
            # We herein remove unneeded parts.
            
            pad_length = max_length_of_this_batch - item[2][0].shape[-1]
            ...
            obs_time_seq = np.pad(item[2][0], ((0, 0), (0, pad_length)), mode = 'constant', constant_values = 0)
            obs_event_seq = np.pad(item[2][1], ((0, 0), (0, pad_length)), mode = 'constant', constant_values = 0)
            obs_padding_mask = np.pad(item[2][2], ((0, 0), (0, pad_length)), mode = 'constant', constant_values = 0)
            obs_interval_has_missing_events = convert_missing_mask_to_gap_mask(item[2][3])
            obs_interval_has_missing_events = torch.nn.utils.rnn.pad_sequence([torch.tensor(item, dtype = torch.int64) for item in obs_interval_has_missing_events], batch_first = True)
            # obs_interval_has_missing_events is one item shorter than the correspounding observed sequence.
            # We must add the missing item back.
            padded_missing_mark = np.pad(obs_interval_has_missing_events, ((0, 0), (0, pad_length + 1)), mode = 'constant', constant_values = 0)
            padded_missing_mark_mask = remove_the_last_event_from_mask(obs_padding_mask)
            ...
            # obs_time_seq_backward = np.pad(item[3][0], ((0, 0), (0, pad_length)), mode = 'constant', constant_values = 0)
            
            padded_obs_time.append(obs_time_seq)
            padded_obs_event.append(obs_event_seq)
            padded_obs_mask.append(obs_padding_mask)
            padded_interval_has_missing.append(padded_missing_mark)
            padded_interval_has_missing_mask.append(padded_missing_mark_mask)
               
        from torch.utils.data._utils.collate import default_collate
        
        padded_obs_time = default_collate(padded_obs_time)
        padded_obs_event = default_collate(padded_obs_event)
        padded_obs_mask = default_collate(padded_obs_mask)
        padded_interval_has_missing = default_collate(padded_interval_has_missing)
        padded_interval_has_missing_mask = default_collate(padded_interval_has_missing_mask)

        return padded_obs_time, padded_obs_event, padded_obs_mask, padded_interval_has_missing, padded_interval_has_missing_mask, \
               (self.mean, self.std)
