...
from .utils import max_triangle_2d, concat_pad_mat, find_alignment


class ConsensusDecoder(Decoder):
    def __init__(self, del_cost, n_types, max_iter = 5, trans_cost=1.0):
        """
    ...
    def decode_per_del_cost(self, cost_idx, time_stamps_per_particle, weights):
        ...
        for time_stamp, type_idx in decoded_tokens:
            mark.append(type_idx)
            time.append(time_stamp)
        
        mark = np.array(mark, dtype = np.int32)
        time = np.array(time, dtype = np.float32)

        return [mark, time], total_risk
    ...
    def decode(self, particles, weights, retain_risk=False):
        ...
        """
        # Consensus decoder only accepts absolute timestamp.
        particles = [(item[0], np.cumsum(item[1], axis = -1)) for item in particles]
        seq_end_time = np.mean([item[1][0, -1] for item in particles])
        ...
        for particle in particles:
            ...
            particle = np.stack(particle, axis = -1).squeeze()                 # [seq_len, 2]
            for mark, time in particle:
                if mark >= self.n_types:
                    continue
                time_stamps_per_type[int(mark)].append(time)
        ...
        for cost_idx in range(self.n_cost):
            (decoded_mark, decoded_time), risk = self.decode_per_del_cost(cost_idx, time_stamps_per_particle, weights)
            decoded_mark = np.concatenate((np.array([self.n_types]), decoded_mark, np.array([self.n_types])))
            decoded_time = np.concatenate((np.array([0.0]), decoded_time, np.array([seq_end_time])))
            rst.append([decoded_mark, decoded_time])
            risks[cost_idx] = risk
