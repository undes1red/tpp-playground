...
class CTLSTM_barebone(nn.Module):
    def __init__(self, device, num_events, history_module_name, d_mark_embedding, d_input, d_hidden, \
        ...
        '''
        super(CTLSTM_barebone, self).__init__()
    ...
    def forward(self, time_history, events_history):
    ...
    def get_decayed_state(self, history, time_next, num_dimension_prior_batch = 0):
    ...
    def get_decayed_state_of_a_interval(self, history, time_interval_start, time_interval_length, num_dimension_prior_batch = 0):
        eta = self.start_layer(history)                                        # [batch_size, seq_len, d_input]
        mu = self.converge_layer(history)                                      # [batch_size, seq_len, d_input]
        gamma = self.decay_layer(history)                                      # [batch_size, seq_len, d_input]
