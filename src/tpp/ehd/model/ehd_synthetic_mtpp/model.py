...
from src.ehd.model.ehd_synthetic_mtpp.submodel import EHD_backend
from src.ehd.model.basic_ehd_model import BasicModule, check_tensor, move_from_tensor_to_ndarray
from src.ehd.model.ehd_synthetic_mtpp.plot import * 
from src.ehd.model.ehd_synthetic_mtpp.nes.nes import NES
from src.ehd.model.ehd_synthetic_mtpp.syn_function_repo import syn
...
class EHD(BasicModule):
    ...
    def __init__(self, opt, training, d_input, d_rnn, d_hidden, n_layers_encoder, \
                 ...
                 device, additional_model, samples_for_l_p = 32, minimal_L1 = 6):
        ...
        self.length_of_h = opt.info_dict['length_of_h']

        self.model = EHD_backend(num_events = self.num_events, d_input = d_input, d_rnn = d_rnn, d_hidden = d_hidden, \
        ...
        self.nes_module = NES(num_of_samples_mask = self.samples_for_l_p, length_of_h = self.length_of_h, \
                              first_event_in_his_is_dummy = True, epsilon = self.epsilon, tau = 0.1, device = self.device)
        
        self.synthetic_model = syn(func = additional_model['function'], minimal_L1 = self.minimal_L1, \
                                   device = self.device, **additional_model['kwargs'])
    ...
    def train_procedure(self, input_time, input_events, input_expanded_probability, input_mask, mean, std):
        assert (input_mask == 1).all()
        number_of_events = input_mask.sum().item()

        cum_input_time = input_time.cumsum(dim = -1)                           # [batch_size, seq_len]
        batch_size = input_time.shape[0]

        (L_c, L_g), history_mask, (padded_filtered_masks,), (padded_filtered_cum_input_time,) \
            = self.nes_module(self.model, (input_time, input_mask, mean, std), \
                              input_mask, number_of_events, \
                              discrete_inputs = (input_mask,), \
                              continuous_inputs = (cum_input_time,),)
                                                                               # [samples_for_l_p * batch_size, *]
        padded_filtered_cum_input_time = rearrange(padded_filtered_cum_input_time, '(sr b) ... -> sr b ...', sr = self.samples_for_l_p)
                                                                               # [samples_for_l_p, batch_size, *]
        gap_selected = self.synthetic_model(time_history = padded_filtered_cum_input_time, \
                                            true_probability = input_expanded_probability, 
                                            selection_mask = history_mask[..., 1])
                                                                               # [samples_for_l_p, batch_size, *]
        L_p = gap_selected.mean()
        Loss = self.lambda_l_c * L_c + self.lambda_l_p * L_p

        return Loss, L_c, L_p, L_g


    def evaluate_procedure(self, input_time, input_events, input_expanded_probability, 
                           input_mask, mean, std):
        '''
        ...
        (L_c, L_g), history_mask, (padded_filtered_masks,), (padded_filtered_cum_input_time,) \
            = self.nes_module(self.model, (input_time, input_mask, mean, std), \
                              input_mask, number_of_events, \
                              discrete_inputs = (input_mask,), \
                              continuous_inputs = (cum_input_time,), evaluate = True)
        
        _, _, (padded_selected_filtered_masks,), (padded_selected_filtered_cum_input_time,) \
            = self.nes_module(self.model, (input_time, input_mask, mean, std), \
                              input_mask, number_of_events, \
                              discrete_inputs = (input_mask,), \
                              continuous_inputs = (cum_input_time,), \
        ...
        gap_with_noise_events = self.synthetic_model(time_history = padded_filtered_cum_input_time, \
                                                     true_probability = input_expanded_probability, 
                                                     selection_mask = history_mask[..., 0])

        gap_with_essential_events = self.synthetic_model(time_history = padded_selected_filtered_cum_input_time, \
                                                         true_probability = input_expanded_probability, 
                                                         selection_mask = history_mask[..., 1])
        
        gap_with_noise_events = gap_with_noise_events.mean()
        L_p = gap_with_essential_events.mean()
        Loss = self.lambda_l_c * L_c + self.lambda_l_p * L_p

        return Loss, L_c, L_p, L_g, gap_with_noise_events
    ...
    def train_step(model, minibatch, device):
        ...
        model.train()
        (input_time, input_events, input_score, input_mask, input_expanded_probability), (mean, std) = minibatch
        loss, L_c, L_p, L_g = model(         
                task_name = 'train', input_time = input_time, input_events = input_events, \
                input_expanded_probability = input_expanded_probability, input_mask = input_mask, \
                mean = mean, std = std
        )
    ...
    def evaluation_step(model, minibatch, device):
        ...
        model.eval()
        (input_time, input_events, input_score, input_mask, input_expanded_probability), (mean, std) = minibatch
        loss, L_c, L_p, L_g, gap_with_noise_events = model(         
        ...
        L_p = L_p.item()
        L_g = L_g.item()
        gap_with_noise_events = gap_with_noise_events.item()
    ...
    def log_print_format(input, procedure):
        ...
        def test_log_print_format(input):
            ...
            format_dict['L_p'] = input[2]
            format_dict['L_g'] = input[3]
            format_dict['L_p_of_noise_events'] = input[4]
            format_dict['num_format'] = {'Loss': ':6.5f', 'L_c': ':6.5f', 'L_p': ':6.5f', 'L_g': ':6.5f', 
                                         'L_p_of_noise_events': ':6.5f'}
            return format_dict
