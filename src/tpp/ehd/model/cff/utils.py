...
def generate_masks(input_probability, seq_len_x, evaluate=False):
    ...
    filter_mask = torch.cat(
        [history_mask, future_mask], dim=-2
    )  # [num_of_samples_mask, batch_size, length_of_h + length_of_x + ?, 2]
    filter_mask[:, :, 0] = 1
...
def filter(discrete_inputs, continuous_inputs, filter_mask, evaluate=False):
    ...
    """
    assert filter_mask is not None, (
        "You want to filter the existing history following the filter mask, but filter mask is unavailable!"
    )
    ...
    We use discrete_filter_mask_for_distilled_marks to select data after we multiply filter_mask_for_distilled_marks
    ...
    one_tensor_for_gradient_attachment = torch.take_along_dim(
        filter_mask, select_value_for_gradient_attachment, dim=-1
    ).squeeze(dim=-1)
    # [num_of_samples_mask, batch_size, seq_len]
    discrete_filter_mask_for_left_marks = filter_mask[..., 0].detach().int()
    # [num_of_samples_mask, batch_size, seq_len]
    discrete_filter_mask_for_distilled_marks = filter_mask[..., 1].detach().int()
    # [num_of_samples_mask, batch_size, seq_len]
    the_number_of_distilled_mark = discrete_filter_mask_for_distilled_marks.sum(dim=-1).flatten().tolist()
    # [num_of_samples_mask, batch_size]
    the_number_of_left_mark = discrete_filter_mask_for_left_marks.sum(dim=-1).flatten().tolist()
    ...
    def gradient_attachment(x):
        if len(x.shape) == len(one_tensor_for_gradient_attachment.shape):
            ...
            return x * rearrange(
                one_tensor_for_gradient_attachment, einop
            )  # [num_of_samples_mask, batch_size, seq_len, ...]
        else:
            raise Exception("Mask has more dimension than the input tensor, which is unexpected.")

    def select_marks_by_mask(x):
        if len(x.shape) >= len(discrete_filter_mask_for_distilled_marks.shape):
            min_shape_len = min(len(x.shape), len(discrete_filter_mask_for_distilled_marks.shape))
            # [num_of_samples_mask, batch_size, seq_len, ...]
            assert x.shape[:min_shape_len] == discrete_filter_mask_for_distilled_marks.shape[:min_shape_len], (
                "Dimension of input and mask tensor mismatches, selection can not continue."
            )
            return x[discrete_filter_mask_for_distilled_marks == 1], x[discrete_filter_mask_for_left_marks == 1]
        else:
            raise Exception("Mask has more dimension than the input tensor, which is unexpected.")

    # select the remained marks from the original input.
    continuous_inputs = map(
        gradient_attachment, continuous_inputs
    )  # [num_of_samples_mask, batch_size, seq_len, (...)] * n
    selected_discrete_inputs = map(select_marks_by_mask, discrete_inputs)  # [(...) * 2] * m
    selected_continuous_inputs = map(select_marks_by_mask, continuous_inputs)
    # [(...) * 2] * n
    padded_distilled_discrete_inputs = []
    padded_left_discrete_inputs = []
    for selected_discrete_input in selected_discrete_inputs:
        padded_distilled_discrete_input, padded_left_discrete_input = regenerate_batch(
            selected_discrete_input, (the_number_of_distilled_mark, the_number_of_left_mark), num_of_samples_mask
        )
    ...
    for selected_continuous_input in selected_continuous_inputs:
        padded_distilled_continuous_input, padded_left_continuous_input = regenerate_batch(
            selected_continuous_input, (the_number_of_distilled_mark, the_number_of_left_mark), num_of_samples_mask
        )
...
def regenerate_batch(input_seqs, the_number_of_marks, num_of_samples_mask):
    distilled_features, left_features = input_seqs
    the_number_of_distilled_mark, the_number_of_left_mark = the_number_of_marks

    distilled_features = distilled_features.split(the_number_of_distilled_mark, dim=0)
    # (num_of_samples_mask * batch_size) * (*)
    left_features = left_features.split(the_number_of_left_mark, dim=0)  # (num_of_samples_mask * batch_size) * (*)
