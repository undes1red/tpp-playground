...
from src.ehd.model.ehd_synthetic_new.nes.utils import *
...
class NES(nn.Module):
    ...
    def forward(self, probability_model, model_args, discrete_inputs, continuous_inputs, evaluate = False, pick_essential_events = False):
        '''
        ...
        input_probability = probability_model(*model_args)                     # [batch_size, seq_len, 2]
        check_tensor(input_probability)
        batch_size, seq_len, _ = input_probability.shape
        ...
        gap_between_p_1_and_p_0 = input_probability[:, :, 1] - input_probability[:, :, 0]
                                                                               # [batch_size, seq_len]
        gap_mean = torch.abs(gap_between_p_1_and_p_0).mean()
        ...
        if evaluate:
            ...
            history_mask = F.gumbel_softmax(
                torch.log(repeated_input_probability + self.epsilon), tau = 1.0, hard = True, dim = -1)
                                                                               # [samples_for_l_p, batch_size, seq_len, 2]
        check_tensor(history_mask)
        '''
        Loss 1: L_c, optimize the length of essential events.
        '''
        selected_mask = history_mask[..., 0]
        L_c = torch.linalg.norm(selected_mask.float(), ord = 1, dim = -1) / seq_len
                                                                               # [num_of_samples_mask, batch_size]
        L_c = L_c.mean()

        padded_selected_discrete_inputs, padded_selected_continuous_inputs \
            = self.filter(discrete_inputs, continuous_inputs, filter_mask = history_mask, evaluate = evaluate, pick_essential_events = pick_essential_events)
