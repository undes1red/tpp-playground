...
from src.tpp.tpp_models.utils import step_split
...
# For sampling_by_its.
def sampling_by_its_for_mt(self, time_history, marks_history, p_m, number_of_total_samples, step, inf_val, mean, std):
    # Preprocess
    sample_rate_list = step_split(number_of_total_samples, step)
    ...
    def bisect_target(taus, probability_threshold):
        ...
        probability_integral_from_t_to_infinite, _ = self.model(
            time_history, taus, marks_history, mean, std, extend_input_time=False
        )
...
def sampling_by_its_for_tm(self, time_history, marks_history, number_of_total_samples, step, mean, std):
    ...
    def evaluate(taus, probability_threshold, integral_from_zero_to_inf):
        probability_integral_from_t_to_inf, _ = self.model(time_history, taus, marks_history, mean, std)
        # [sample_rate, batch_size, seq_len, num_marks]
        # P_m(t) = \\int_{0}^{t}{p(t|m, \\mathcal{H})}
        probability_integral = integral_from_zero_to_inf - probability_integral_from_t_to_inf
        # [sample_rate, batch_size, seq_len, num_marks]
    ...
    for sub_sample_rate in sample_rate_list:
        ...
        # [sub_sample_rate, batch_size, seq_len]
        integral_from_zero_to_inf, _ = self.model(time_history, time_next_zero, marks_history, mean=mean, std=std)
        # [sub_sample_rate, batch_size, seq_len, num_marks]
...
def sample_time_mark(
    ...
    while True:
        ...
        # [number_of_sampled_sequences]
        repeated_sampled_time = repeat(sampled_time, "... -> ... ne", ne=self.num_marks)  # [number_of_sampled_sequences, num_marks]
        repeated_sampled_time.requires_grad = True
        integral_from_sampled_time_to_inf = self.model.gamma_at_t_autoregressive(
            time_history_for_sampling, repeated_sampled_time, marks_history_for_sampling, mean=mean, std=std, extend_input_time=False
        )
