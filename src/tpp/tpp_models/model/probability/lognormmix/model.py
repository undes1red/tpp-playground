...
from src.toolbox.misc import argument_check, move_from_tensor_to_ndarray, pack_one_value_to_dict
...
class LogNormMixWrapper(BasicModel):
    ...
    def figure_intensity(self, input_data, opt):
        ...
        return NotImplementedError(
            "LogNormMix is intensity-free. Therefore, it can not provide the plot for the intensity function."
        )
    ...
    def figure_integral(self, input_data, opt):
        ...
        """
        return NotImplementedError(
            "LogNormMix is intensity-free. Therefore, it can not provide the plot for the intensity integral."
        )
    ...
    def figure_probability(self, input_data, opt):
        ...
        input_time_for_generating_reference = torch.cat(
            (torch.zeros(batch_size, 1, device=self.device), input_time[:, :-1]), dim=-1
        )
        input_events_for_generating_reference = torch.cat(
            (torch.ones(batch_size, 1, device=self.device, dtype=torch.int) * self.num_events, input_events[:, :-1]),
            dim=-1,
        )
        input_mask_for_generating_reference = torch.cat(
            (torch.ones(batch_size, 1, device=self.device, dtype=torch.int), input_mask[:, :-1]), dim=-1
        )
        ...
        # [batch_size, seq_len]
        expand_probability, timestamp = self.model.probability_prober(
            input_events, input_time, input_mask, opt.resolution, mean, std
        )
    ...
    def figure_debug(self, input_data, opt):
        ...
        data = {"mask_next": mask_next, "mae": mae}
    ...
    def train_step(model, minibatch, device):
        ...
        time_loss, time_loss_without_dummy, events_loss, the_number_of_events = model(
            task_name="train", **extract_minibatch(minibatch)
        )
    ...
    def evaluation_step(model, minibatch, device):
        ...
        time_loss, surv_last_loss, event_loss, mae, f1, the_number_of_events = model(
            task_name="evaluate", **extract_minibatch(minibatch)
        )
