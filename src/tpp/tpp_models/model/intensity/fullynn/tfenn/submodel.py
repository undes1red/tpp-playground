...
class TFENN(nn.Module):
    ...
    def history_seq_encoding(self, time_history, marks_history, mean, std, custom_marks_history=False):
        ...
        batch_size, seq_len = time_history.shape[-2:]

        time_history = repeat(time_history, "... -> ... ne", ne=self.num_marks)  # [batch_size, seq_len, num_marks]
        time_embeddings = torch.zeros(batch_size, seq_len, self.d_intensity, device=self.device)  # [batch_size, seq_len, d_intensity]
        ...
        return time_embeddings  # [batch_size, seq_len, d_intensity]
    ...
    def forward(self, time_history, time_next, marks_history, mask_history, mean, std, training=False):
        ...
        """
        time_history_embedding = self.history_seq_encoding(time_history, marks_history, mean, std)
        # [batch_size, seq_len, d_intensity]
        ...
        hidden_history = rearrange(hidden_history, "b s dh -> b s () dh")
        # [batch_size, seq_len, num_marks, d_intensity]
    ...
    def integral_time_next_2d(
        ...
        hidden_history = rearrange(hidden_history, "b s di -> b s () () di")
    ...
    def model_probe_function(
        ...
        hidden_history = self.history_mapper(hidden_history)
        # [batch_size, seq_len, d_intensity]
