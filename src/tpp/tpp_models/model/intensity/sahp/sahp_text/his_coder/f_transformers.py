...
class Encoder(nn.Module):
    ...
    def forward(self, event_time, event_type, text_emb, non_pad_mask, custom_mark_history):
        """
        ...
        if self.d_text_emb < self.d_input:
            text_emb = torch.nn.functional.pad(
                text_emb, (0, self.d_input - self.d_text_emb, 0, 0, 0, 0), mode="constant", value=0
            )  # [batch_size, seq_len, d_input]
        else:
            text_emb = text_emb[..., : self.d_input]  # [batch_size, seq_len, d_input]

        output = time_emb + mark_emb + text_emb  # [batch_size, seq_len, d_input]
        for enc_layer in self.layer_stack:
            output = enc_layer(output, non_pad_mask=non_pad_mask)  # [batch_size, seq_len, d_input]
        return output
...
class TransformerTPP(nn.Module):
    ...
    def forward(self, event_time, event_type, text_emb, non_pad_mask, custom_marks_history):
        """
        Encode the input continuous-time event stream using Transformer.
        ...
        ### Outputs
            * ```torch.tensor``` enc_output
              ...
              The representation of the original input.
        """
        return self.encoder(event_time, event_type, text_emb, non_pad_mask, custom_marks_history)
        # [batch_size, seq_len, d_input]
