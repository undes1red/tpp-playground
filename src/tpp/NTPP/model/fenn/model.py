...
from src.NTPP.model.fenn.sample import sample_time
from src.NTPP.model.basic_tpp_model import BasicModel, memory_ceiling
from src.NTPP.model.fenn.submodel import FENN
from src.NTPP.model.fenn.plot import *
from src.NTPP.model.utils import predict_event, decide_resolution_inf_and_resolution_between_events, get_f1_and_top_k_acc_in_mae_e, pick_log_probability
...
class FENNModel(BasicModel):
    ...
    def __init__(self, d_history,
                 ...
                 sample_rate = 32, mtpp_note_global_known = False, mtpp_note_is_true_note = False, mtpp_includes_note_embedding = True, 
                 mtpp_not_conditioned_on_note = False):
        '''
        ...
        self.model = FENN(d_history = d_history, d_intensity = d_intensity, num_events = self.num_events,
                          ...
                          mlp_layers = mlp_layers, device = device, mtpp_includes_note_embedding = self.mtpp_includes_note_embedding, \
    ...
    def train_procedure(self, input_time, original_time_seq, input_events, original_events, note, note_embedding, mask, mean, std, step):
        '''
        ...
        original_time_history, original_time_next = self.divide_history_and_next(original_time_seq)
        ...
        integral_for_each_event, predicted_note_embedding = \
            self.model(events_history, time_history, time_next, mask_history, mean = mean, std = std, \
        ...
        if self.mtpp_includes_note_embedding and not self.mtpp_not_conditioned_on_note:
            mse_loss = torch.nn.functional.mse_loss(input = predicted_note_embedding, target = note_embedding_next, reduction = 'none')
        ...
        loss = time_loss_without_dummy + loss_survival + mse_loss

        return loss, time_loss_without_dummy, mse_loss, events_loss, the_number_of_events


    def evaluate_procedure(self, input_time, original_time_seq, input_events, original_events, note, note_embedding, mask, mean, std):
        '''
        ...
        # preparing for multi-event training when needed
        time_next.requires_grad = True
        integral_for_each_event_from_tl_to_time_next, predicted_note_embedding = \
            self.model(events_history, time_history, time_next, mask_history, mean = mean, std = std, \
                       note_embedding_history = note_embedding_history, output_pred_note_embedding = True, 
                       note_embedding_next = note_embedding_next)              # [batch_size, seq_len, num_events]
        ...
        return time_loss + integral_survival.sum(), loss_survival, events_loss, mse_loss, mae, f1, the_number_of_events
    ...
    def mean_absolute_error_and_f1(self, events_history, time_history, events_next, time_next, mask_history, mask_next, \
                                   mean, std, opt = None, note_embedding_history = None, note_embedding_next = None, output_pred_note_embedding_mse = False):
        '''
        ...
        if output_pred_note_embedding_mse:
            integral_for_each_event, note_pred_emb = self.model(events_history, time_history, time_next, mask_history, mean = mean, std = std, \
                                                                note_embedding_history = note_embedding_history, note_embedding_next = note_embedding_next, \
                                                                output_pred_note_embedding = output_pred_note_embedding_mse)
                                                                               # [batch_size, seq_len, num_events]
            mse = torch.pow(note_embedding_next - note_pred_emb, 2)            # [batch_size, seq_len, note_embedding_size]
        ...
        if output_pred_note_embedding_mse:
            return mae, f1, events_probability, mse
        else:
    ...
    def train_step(model, minibatch, device, step):
        ...
        (time_seq, original_time_seq, event_seq, original_events, note, note_embedding, score, mask), (mean, std) = minibatch
                                                                               # 6 * [batch_size, seq_len + 1] + two floats
        loss, time_loss_without_dummy, mse_loss, events_loss, the_number_of_events = \
        model(task_name = 'train', input_time = time_seq, original_time_seq = original_time_seq, \
              input_events = event_seq, original_events = original_events, note = note, note_embedding = note_embedding, mask = mask, mean = mean, std = std, step = step)
    ...
    def evaluation_step(model, minibatch, device):
        ...
        (time_seq, original_time_seq, event_seq, original_events, note, note_embedding, score, mask), (mean, std) = minibatch
                                                                               # 6 * [batch_size, seq_len + 1] + two floats
        time_loss, loss_survival, events_loss, mse_loss, mae, f1, the_number_of_events = model(
                task_name = 'evaluate', input_time = time_seq, original_time_seq = original_time_seq, \
                input_events = event_seq, original_events = original_events, note = note, note_embedding = note_embedding, mask = mask, mean = mean, std = std
        )
