...
from src.tpp.tpp_models.tfullynn.submodel import TFullyNN
from src.tpp.tpp_models.utils import predict_event, get_f1_and_top_k_acc_in_mae_e, decide_resolution_inf_and_resolution_between_events
from src.tpp.tpp_models.tfullynn.plot import *
from src.tpp.tpp_models.tfullynn.sample import sample_time


class TFullyNNModel(BasicModel):
    '''
    The FullyNN with a Transformer as the history encoder.
    '''
    ...
    def evaluate_procedure(self, input_time, input_events, mask, mean, std):
        '''
        TFullyNN's forwardpropagation function for evaluation.
    ...
    def mean_absolute_error_and_f1(self, events_history, events_next, time_history, time_next, mask_history, mask_next,
        ...
        integral_for_each_event = self.model(events_history, time_history, time_next, mask_history = mask_history, mean = mean, std = std)
                                                                               # [batch_size, seq_len, num_events]
        # Obtains intensity values.
        intensity_for_each_event = torch.autograd.grad(
            outputs = integral_for_each_event,
    ...
    def mean_absolute_error_e(self, events_history, events_next, time_history, time_next, mask_history, mask_next,
        ...
        return f1, top_k_acc, probability_integral_sum, p_m, tau_pred_all_event, \
    ...
    def figure_debug(self, input_data, opt):
        ...
        mae, f1_1, _ = self.mean_absolute_error_and_f1(events_history, events_next, time_history, time_next, mask_history, mask_next, mean, std, opt = opt)
                                                                               # [batch_size, seq_len]
        ...
        f1_2, top_k, probability_sum, tau_pred_all_event, p_m, maes_avg, maes \
            = self.mean_absolute_error_e(events_history, events_next, time_history, time_next, mask_history, mask_next, mean, std, return_mean = False, opt = opt)
    ...
    def get_mae_e_and_f1(self, input_data, opt):
        ...
        f1_2, top_k, probability_sum, p_m, tau_pred_all_event, maes_avg, maes \
            = self.mean_absolute_error_e(events_history, events_next, time_history, time_next, mask_history, mask_next, mean, std, opt = opt)
