...
from src.tpp.tpp_models.llmtpp_repro.submodel import LLMTPP
from src.tpp.tpp_models.utils import *
from src.tpp.tpp_models.llmtpp_repro.plot import *


class LLMTPPModel(BasicModel):
    def __init__(self, opt, llm_class_name, full_llm_name, d_model, \
                 d_embedding, repro_input_layer, device, dropout, epsilon = 1e-20, lambda_t = 1.0, lambda_e = 1.0):
        ...
        self.model = LLMTPP(llm_class_name = llm_class_name, full_llm_name = full_llm_name, \
                            ...
                            repro_input_layer = repro_input_layer, dropout = dropout, device = device)
    ...
    def get_resampling_weight_against_next_num(self, event_counts, event_next):
        max_count = torch.max(event_counts)
        resample_weight = max_count / event_counts                             # [num_events]
        # In case some rare events do not present in this batch causing the resample_weight to be inf.
        resample_weight = torch.nan_to_num(resample_weight, nan = 0.0, posinf = 0.0, neginf = 0.0)
                                                                               # [num_events]
        return (resample_weight * torch.nn.functional.one_hot(event_next, num_classes = self.num_events)).sum(dim = -1)
        
    
    def event_count_in_event_next(self, events_next_without_dummy, mask_next_without_dummy):
        event_counts = torch.tensor([0] * self.num_events, device = self.device)
                                                                               # [num_events]
        for events_next_without_dummy_per_batch, mask_next_without_dummy_per_batch in \
            zip(events_next_without_dummy, mask_next_without_dummy):
            selected_events = events_next_without_dummy_per_batch[mask_next_without_dummy_per_batch]
            marks, num_of_marks = selected_events.unique(return_counts = True)
            for mark, num in zip(marks, num_of_marks):
                event_counts[mark] += num
        
        return event_counts


    def loss(self, pred_time, time_next, event_next, mask_next, resample = None):
        '''
