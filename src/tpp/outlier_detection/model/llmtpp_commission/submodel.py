...
from src.OD.model.llmtpp_commission.transformers_module import lm_module_location
from src.OD.model.llmtpp_commission.embedding import DataEmbedding
...
class LLMTPP(nn.Module):
    ...
    def model_forward(self, input_time, input_events, input_seq_mask, mean, std):
        input_time = (input_time - mean) / std                                 # [batch_size, seq_len, d_model]
        ...
        outputs = self.retrieved_lm(inputs_embeds = input_embs).last_hidden_state
                                                                               # [batch_size, seq_len, d_lm_model]
        mark_dist = self.missing_score(outputs)                                # [batch_size, seq_len, 1]
        missing_score = torch.nn.functional.sigmoid(mark_dist).squeeze()       # [batch_size, seq_len]
