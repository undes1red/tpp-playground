...
from src.OD.model.llmtpp_commission.submodel import LLMTPP
from src.OD.model.utils import *
from src.OD.model.llmtpp_commission.plot import *
...
class LLMTPPModel(BasicModel):
    ...
    def forward(self, task_name, *args, **kwargs):
        ...
        * evaluate      type: bool shape: N/A
        ...
        task_mapper = {
            ...
            'omission_roc_auc': self.commission_outlier,
        }

        return task_mapper[task_name](*args, **kwargs)


    def train_procedure(self, input_time, input_events, commission_label, mask, mean, std):

        missing_score = self.model('train', input_time = input_time, input_events = input_events, \
                                   input_seq_mask = mask, mean = mean, std = std)
                                                                               # [batch_size, seq_len]
        
        loss = torch.nn.functional.binary_cross_entropy(missing_score, commission_label.float(), reduction = 'none')
                                                                               # [batch_size, seq_len]
    ...
    def evaluate_procedure(self, input_time, input_events, commission_label, input_seq_mask, mean, std):
        ...
        loss = torch.nn.functional.binary_cross_entropy(missing_score, commission_label.float(), reduction = 'none')
                                                                               # [batch_size, seq_len]
        masked_loss = loss * input_seq_mask
        loss = masked_loss.sum() / input_seq_mask.sum()
        
        # Metric: roc_auc.
        packed_data = zip(missing_score, commission_label, input_seq_mask)
        all_roc_auc = []
        for (missing_score_per_batch, commission_label_per_batch, input_seq_mask_per_batch) \
            in packed_data:

            selected_missing_score = missing_score_per_batch[input_seq_mask_per_batch == 1]
            selected_commission_label = commission_label_per_batch[input_seq_mask_per_batch == 1]
            selected_commission_label, selected_missing_score = move_from_tensor_to_ndarray(selected_commission_label, selected_missing_score)
            all_roc_auc.append(roc_auc_score(y_true = selected_commission_label, y_score = selected_missing_score))
    ...
    def commission_outlier(self, input_data, opt):
    ...
    def train_step(model, minibatch, device):
        ...
        model.train()
        (padded_time_seq, padded_event, padded_commission, mask), (mean, std) = minibatch
        
        '''
        Here, we need the complete forward data and the backward time.
        '''
        loss = model(task_name = 'train', \
                     input_time = padded_time_seq, \
                     input_events = padded_event, \
                     commission_label = padded_commission, \
                     input_seq_mask = mask, \
                     mean = mean, \
                     std = std)
