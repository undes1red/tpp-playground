...
if __name__ == '__main__':
    ...
    result = metric.compare(first_metric)
    assert result == True, 'Why do not accept the first group of metric values?'
    assert metric.show() == [1,], 'We stored the wrong metric values!'
    print(metric.show())
    
    second_metric = [0.5] * 1
    result = metric.compare(second_metric)
    assert result == True, 'This one should be better!'
    assert metric.show() == [0.5,], 'We stored the wrong metric values!'
    print(metric.show())

    second_metric = [0.25] * 1
    result = metric.compare(second_metric)
    assert result == True, 'This one should be better!'
    assert metric.show() == [0.25,], 'We stored the wrong metric values!'
    print(metric.show())
