...
def kl_divergence_generic_torch(p, q, dim):
    if len(p.shape) != len(q.shape):
        raise Exception(f"The dimension of p {len(p.shape)} mismatches q {len(q.shape)}.")

    dim_num = len(p.shape)
    if dim != -1 or dim != dim_num - 1:
        dim = dim_num + dim + 1 if dim < 0 else dim + 1
        einop = " ".join([f"a{i}" for i in range(dim)]) + ' ... -> ' + " ".join([f"a{i}" for i in range(dim - 1)]) + f' ... a{dim - 1}'
...
def kl_divergence_generic_numpy(p, q, dim):
    if len(p.shape) != len(q.shape):
        raise ValueError(f"The dimension of p {len(p.shape)} mismatches q {len(q.shape)}.")
    dim_num = len(p.shape)
    if dim != -1 or dim != dim_num - 1:
        dim = dim_num + dim + 1 if dim < 0 else dim + 1
        einop = " ".join([f"a{i}" for i in range(dim)]) + ' ... -> ' + " ".join([f"a{i}" for i in range(dim - 1)]) + f' ... a{dim - 1}'
