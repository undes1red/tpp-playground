...
def compile_model(model, use_compile, *args, **kwargs):
    ...
    return torch.compile(model, *args, disable=not use_compile, **kwargs)
...
def conditional_compile_class_method(func):
    def wrapper(*args, **kwargs):
        ...
        if hasattr(args[0], "compile_or_not"):
            ...
            logger.debug(
                "compile_or_not undefined in the class object! For compatibility, we will not compile the decorated function."
            )
