...
class CustomOpenAIforVLLM:
    ...
    def completions(self, prompt_list, stream = False, n_threads = 1, **kwargs):
        ...
        if n_threads > 1:
            with ThreadPoolExecutor(max_workers = n_threads) as executor:
                results_in_future = {idx: executor.submit(self.completion, prompt, stream, **kwargs) for idx, prompt in enumerate(prompt_list)}
    ...
    def tokenize(self, input, full_output = False):
        headers = {
    ...
    def detokenize(self, input, full_output = False):
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_key}"
        }
...
if __name__ == '__main__':
    ...
    # model = CustomOpenAI(url_path, model_name, device = 'cuda:0', api_key = api_key)
