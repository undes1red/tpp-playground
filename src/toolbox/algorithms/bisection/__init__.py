...
def bisection(max_step, bisect_early_stop_threshold, bisect_func, threshold, *args, resolution=3, **kwargs):
    if torch.is_tensor(threshold):
        return bisection_torch(max_step, bisect_early_stop_threshold, resolution, bisect_func, threshold, *args, **kwargs)
    return bisection_numpy(max_step, bisect_early_stop_threshold, resolution, bisect_func, threshold, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    class TestBisectionMethods(unittest.TestCase):
        ...
        def test_bisection_numpy_1(self):
            ...
            result = bisection_numpy(100, 1e-6, 3, test_func, probability_threshold, l_val=0, r_val=1)
            self.assertAlmostEqual(result[0], 0.5, places=6)

        def test_bisection_torch_1(self):
            """Test basic bisection with simple function"""
            probability_threshold = torch.tensor([0.5])
            result = bisection_torch(100, 1e-6, 3, test_func, probability_threshold, l_val=0, r_val=1)
        ...
        def test_bisection_numpy_3(self):
            ...
            result = bisection_numpy(100, 1e-6, 3, test_func, probability_threshold, l_val=0.5, r_val=1)
            self.assertAlmostEqual(result[0], 0.9, places=6)

        def test_bisection_torch_3(self):
            """Test with different range"""
            probability_threshold = torch.tensor([0.9])
            result = bisection_torch(100, 1e-6, 3, test_func, probability_threshold, l_val=0.5, r_val=1)
