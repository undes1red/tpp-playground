...
def possible_checkpoint_detect(opt, id):
    ...
    for possible_valid_dir_name in possible_valid_dir_names:
        possible_checkpoint = os.path.join(tmp_path, str(possible_valid_dir_name), opt.training_dataset_name if opt.training_dataset_name is not None else opt.dataset_name, folder_name, 'checkpoint.chkpt')
