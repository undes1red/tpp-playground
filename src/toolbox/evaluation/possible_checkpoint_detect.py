...
def possible_checkpoint_detect(opt, idx):
    ...
    # Scan valid folders.
    tmp_path = opt.root_path / "model" / opt.procedure_path
    files = os.scandir(tmp_path)
    possible_valid_dir_names = [
        int(dir_item.name) for dir_item in filter(lambda x: not x.is_file() and x.name.isdigit(), files)
    ]
    valid_dir_indexes = []
    ...
    for possible_valid_dir_name in possible_valid_dir_names:
        possible_checkpoint = (
            tmp_path
            / str(possible_valid_dir_name)
            / opt.dataset_name_path
            / folder_name
            / "checkpoint.chkpt"
        )
        print(possible_checkpoint)
