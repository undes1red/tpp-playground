...
from src.toolbox.bulk_task_runner import parameter_parser, monitor_and_automaticly_run_tasks
from src.toolbox.misc import get_logger, read_yaml, mkdir_if_not_exist
...
parser.add_argument(
    ...
    nargs="+",
    default=None,
    help='Tell us which job do you want to execute. This argument accepts multiple inputs so you can use "--job_name A B C" to run job A B C one by one. \n \
...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="The number of tasks we should run in parallel. In GPU mode this number should not bigger than the number of available GPUs. \
...
stdout_dir = os.path.join(
    root_path, "stdout", opt.procedure_name, opt.script_type, opt.model
)
...
def task_generator(hyperparameter_list: Dict[str, Any]) -> tuple[List[List], int]:
    ...
    """
    hyperparameter_list["file_name"] = os.path.join(
        root_path, hyperparameter_list["worker"]
    )
...
if opt.script_type == "previous_failed_tasks":
    ...
    try:
        ...
        logger.exception(
            f"File {os.path.join('parameter_set', opt.procedure_name, f'{opt.model}_previous_failed_tasks.txt')} not found!"
        )
    ...
    parameter_lib = importlib.import_module(
        f".{opt.procedure_name}", package="parameter_set"
    )
    ...
    for job in opt.job_name:
        ...
        logger.info(
            f"Current executing the job: {job}. It has {len(job_content)} subjobs."
        )
        ...
        for idx, sub_job in enumerate(job_content):
            ...
            stdout_dir_for_this_subjob = os.path.join(
                stdout_dir, current_local_time, job, "subjob_" + str(idx + 1)
            )
            ...
            if len(failed_tasks) == 0:
                logger.info(
                    f"All {the_number_of_task} tasks have successfully completed."
                )
            else:
                logger.warning(
                    f"{len(failed_tasks)} tasks have failed. Please check what is wrong according to logs in directory stdout/ and fix them!"
